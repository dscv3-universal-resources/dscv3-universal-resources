...
. (Join-Path $root 'sharedScripts' 'Get-TestCase.ps1')

BeforeDiscovery {
    if (-not (Get-Command dsc -ErrorAction Ignore)) {
        if (-not (Test-Path -Path (Join-Path $root 'output' 'PSDSC'))) {
            ...
            Save-PSResource @savePSResourceParams

            $env:PSModulePath += [System.IO.Path]::PathSeparator + (Join-Path $root 'output' 'PSDSC')
        }
    ...
    $testData = Join-Path $root 'resources' $ProjectName 'data' 'testData.psd1'
